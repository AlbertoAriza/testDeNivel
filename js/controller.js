...
const showStatsFunc = function(){
  ...
  if(model.state.counters.questionInLevelNumber === 10 && model.state.counters.rightQuestions[model.state.counters.questionsLevel] >= 8 && model.state.counters.mainCounter < 79){ // lo que se hace si se aprueba un nivel
...
const brainFunction = function(){
  ...
  */
  ...
  }else if(model.state.counters.mainCounter < 69){ // → Render 4 listenings C1
    if(model.state.counters.mainCounter === 68)model.state.counters.showStats = true;
    loadListeningView();

  }else if(model.state.counters.mainCounter === 69 && model.state.counters.showStats){ // Render STATS C1
    showStatsFunc();

  /*
      →→ C2 RENDERS ←←
  */
  }else if(model.state.counters.mainCounter < 79){ // → Mostramos las 10 preguntas de C2
    // render preguntas de C2
    if(model.state.counters.mainCounter === 78)model.state.counters.showStats = true;
    loadGramQuestionView();
